...
const HistorySidebarItem = ({ title, icon, time, status, onItemClick }) => (
  <div 
    className="history-item flex items-center text-xs text-gray-300 py-1 px-2 rounded-md hover:bg-gray-650 cursor-pointer transition-colors duration-150"
    onClick={onItemClick}
  >
    ...
    <span className="title flex-grow truncate hover:text-white" title={title}>{title}</span>
...
const HistorySidebarSection = ({ title, items, onItemClick = () => {} }) => {
  ...
  return (
    <div className="history-section mb-3">
      <div className="section-title text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wider">
        {title}
